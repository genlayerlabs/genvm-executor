...
pub fn handle(args: Args, config: config::Config) -> anyhow::Result<()> {
    ...
    for (id, hashes) in &all {
        for hash in hashes {
            ...
            // v0.2.16 runner ids are the Nix base32 of the tar's sha256 (see
            // `genlayer_sdk::nix32`), which is exactly `Bytes32Hash::to_nix32`.
            let got = genvm_common::Bytes32Hash::from_bytes(digest).to_nix32();
