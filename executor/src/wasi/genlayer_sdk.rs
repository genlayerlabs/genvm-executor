    ...
    async fn gl_call(
        ...
        match request {
            ...
            gl_call::Message::PostMessage {
                ...
                // The emitted envelope leaves this executor: the host runs it as
                // a top-level entry, whose decode contract is the new key set.
                let calldata = match calldata_materialized {
                    Some(mut value) => {
                        super::method_compat::method_legacy_to_new(&mut value);
                        value.into()
                    }
                    None => calldata,
                };
