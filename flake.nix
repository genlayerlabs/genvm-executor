      ...
      let
        ...
        pre-commit-check = git-hooks.lib.${system}.run {
          ...
          hooks = {
            ...
            # --- local guards -----------------------------------------
            # Refuse commits that leave the dev-mode runner toggle on: the
            # legacy line has no check-dev-mode.py, so guard `dev-mode.nix`
            # directly — the file must still contain `false`.
            no-commit-dev-mode = {
              enable = true;
              name = "no-commit-dev-mode";
              entry = "${pkgs.gnugrep}/bin/grep -P false runners/support/versions/dev-mode.nix";
              files = "^runners/support/versions/dev-mode\\.nix$";
              pass_filenames = false;
            };
            # --- commit-msg -------------------------------------------
